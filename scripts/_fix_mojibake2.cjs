...
// Source bytes (in file): C3 AD = �, C3 B3 = �, C2 A1 = �, etc.
...
// Simpler: scan file, find sequences like 0xC3 0xAD (�) / 0xC3 0xB3 (�) / 0xC3 0xA1 (�)
...
for (const f of files) {
  ...
  while (i < buf.length) {
    const b = buf[i];
    // C3 XX pattern that comes from mis-decoded Latin1 (e.g. 0xC3 0xAD -> �)
